            ...
            if (st.time <= 0) {
                ...
                setTimeout(async () => {
                    ...
                    Object.keys(playerStats).forEach(async (userId) => {
                        ...
                        if (user) {
                            if (userStat.amountWon > 0) {
                                user.credits = formatTC((user.credits || 0) + userStat.amountWon);
                                await user.save();
                            }
                            let net = formatTC(userStat.amountWon - userStat.amountBet);
                            if (net !== 0) {
                                await new CreditLog({ username: user.username, action: 'GAME', amount: net, details: roomNames[room] }).save();
                            }
                        }
                    });
            ...
            if (isNewBet) {
                ...
                if (!socket.soloBaseline.active || socket.soloBaseline.game !== data.game) {
                    ...
                    if (amt > spreadLimit && amt > 500) { 
                        return socket.emit('localGameError', { msg: `MARTINGALE CAP: MAX ${formatTC(spreadLimit)} TC ON THIS STREAK`, game: data.game });
                    }
                }
                
                if ((amt * maxPotentialMultiplier) > globalBankVault) {
                    return socket.emit('localGameError', { msg: 'VAULT LIMIT REACHED. CANNOT COVER BET.', game: data.game }); 
                }
