    ...
    for(let d = 0; d < decks; d++) {
        for(let s of ss) {
            for(let v of vs) {
                ...
                shoe.push({ val: v, suit: s, bacVal: bac, bjVal: bj, dtVal: dt, raw: v, suitHtml: `<span class="${colorClass}">${s}</span>` });
            }
        }
    }
...
// =========================================================================
// VIP BLACKJACK MULTIPLAYER ENGINE
// =========================================================================
let mbjState = {
...
io.on('connection', (socket) => {
    ...
    // ... (rest of solo logic and admin logic goes here)
    ...
    // ... (rest of the original Admin & Cashier Handlers remain unchanged) ...
    ...
    socket.on('disconnect', async () => {
        if (socket.user) { 
            ...
            // Refund MBJ if disconnect during BETTING
            for(let i = 1; i <= 5; i++) {
                let s = mbjState.seats[i];
                if (s && s.username === socket.user.username && mbjState.status === 'BETTING') {
                    if (s.hands.length > 0 && s.hands[0].bet > 0) {
